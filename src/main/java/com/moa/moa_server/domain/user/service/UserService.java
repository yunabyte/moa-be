...
public class UserService {
  ...
  public GroupLabelResponse getJoinedGroupLabels(
      Long userId, @Nullable String cursor, @Nullable Integer size) {
    int pageSize = (size == null || size <= 0) ? DEFAULT_PAGE_SIZE : size;
    GroupNameGroupIdCursor parsedCursor =
        cursor != null ? GroupNameGroupIdCursor.parse(cursor) : null;
    ...
    List<Group> groups =
        new LinkedList<>(
            groupMemberRepository.findJoinedGroupLabels(user, parsedCursor, pageSize + 1));
    ...
    String nextCursor =
        groups.isEmpty()
            ? null
            : new GroupNameGroupIdCursor(groups.getLast().getName(), groups.getLast().getId())
                .encode();

    List<GroupLabel> labels = groups.stream().map(GroupLabel::from).toList();

    return new GroupLabelResponse(labels, nextCursor, hasNext, labels.size());
  }

  @Transactional(readOnly = true)
  public JoinedGroupResponse getJoinedGroups(
      Long userId, @Nullable String cursor, @Nullable Integer size) {
    int pageSize = (size == null || size <= 0) ? DEFAULT_PAGE_SIZE : size;
    GroupNameGroupIdCursor parsedCursor =
        cursor != null ? GroupNameGroupIdCursor.parse(cursor) : null;
    ...
    List<GroupMember> members =
        groupMemberRepository.findJoinedGroups(user, parsedCursor, pageSize + 1);
    ...
    String nextCursor =
        members.isEmpty()
            ...
            : new GroupNameGroupIdCursor(
                    members.getLast().getGroup().getName(), members.getLast().getGroup().getId())
                .encode();

    List<GroupDetail> groups = members.stream().map(GroupDetail::from).toList();
