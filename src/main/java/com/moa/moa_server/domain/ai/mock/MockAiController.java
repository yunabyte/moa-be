...
public class MockAiController {
    ...
    public ResponseEntity<ModerationResponse> mockModeration(@RequestBody ModerationRequest request) {
        if (request.content() == null || request.content().isBlank()) {
            return ResponseEntity.badRequest().body(new ModerationResponse("Bad Request", "voteContent must not be null, empty, or whitespace only.", null));
        }
        return ResponseEntity.ok(new ModerationResponse("Accepted", "voteContent has been queued", null));
    }
}
